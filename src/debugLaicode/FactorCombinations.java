...
public class FactorCombinations {
    ...
    // curr: current value
    private void dfs(int index, List<Integer> nums, int target, int curr,
        ...
        for (int i = 0;  curr * (int)Math.pow(n, i) <= target; i++) {
            for (int j = 0; j < i; j++) {
                list.add(n);
            }
            dfs(index + 1, nums, target, curr * (int)Math.pow(n, i), list, res);
            // backtracking
            for (int j = 0; j < i; j++) {
